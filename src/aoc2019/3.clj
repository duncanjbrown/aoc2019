...
(defn find-intersection
  ...
  (if-not (= (line-direction line1) (line-direction line2))
    (let [lines [line1 line2]
          [[vx vay] [_ vby]] (sort-by second (first (filter #(= (line-direction %) :v) lines)))
          [[hax hy] [hbx _]] (sort-by first (first (filter #(= (line-direction %) :h) lines)))]
      (if (and (<= vay hy vby)
               (<= hax vx hbx))
        [vx hy]))))
...
(defn record-2d-line
  ...
  (let [directions {:U [0 1] :D [0 -1] :L [-1 0] :R [1 0]}
        start (last (last state))
        end   (vec (map + start (map * (direction directions) (repeat distance))))
        line [start end]]
    (conj state line)))

(defn parse-step
  [step-string]
  (let [[_ direction count] (re-matches #"(\w)(\d+)" step-string)]
    [(keyword direction) (parse-int count)]))

(defn load-input
  "Get a list of line starts and ends from an input string"
  [input-string]
  (let [steps (map parse-step (clojure.string/split input-string #","))]
    steps))

(defn build-path
  [steps]
  ;; we must drop the [0 0] starting seed
  (drop 1 (reduce record-2d-line [[[0 0]]] steps)))

(defn find-path-intersections
  [path1 path2]
  (for [x path1
        y path2
        :let [intersection (find-intersection x y)]
        :when (and intersection (not= [0 0] intersection))]
    intersection))
...
(defn path-length
  [path]
  (reduce (fn [acc line]
            (+ acc (apply manhattan-distance line))) 0 path))

(defn distance-to-intersection
  [intersection path]
  (let [lines-until-intersection (take-while
                                  (complement (partial line-contains? intersection)) path)
