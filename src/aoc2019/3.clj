...
(defn parallel?
  [line1 line2]
  (= (line-direction line1) (line-direction line2)))
...
(defn manhattan-distance
  [coords]
  (reduce + (map #(Math/abs ^int %) coords)))
...
(defn part1
  ...
  (manhattan-distance
   (first
    (sort-by manhattan-distance (find-path-intersections
                                    (build-path (load-input input1))
                                    (build-path (load-input input2)))))))

(defn part2
  [input1 input2]
  (let [intersections (find-path-intersections (build-path (load-input input1))
                                               (build-path (load-input input2)))]
    intersections))
...
(defn steps-to-intersection
  [intersection path1 path2]
  (let [path1d (take-upto #(line-contains? intersection %) path1)]))
